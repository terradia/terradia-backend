...
export default {
  Query: {
    getUser: async (_parent: any, _args: any, { user }: any) => {
      console.log(user);
      if (!user) {
        return null;
      }
  ...
  Mutation: {
    login: async (_parent: any, {email, password}: any, {secret}: any) => {
    ...
    register: async (_parent: any, {email, ...userInformations}: any, {secret}: any) => {
      ...
      const validationLink = generateAuthlink("checkEmail", {
        id: user.id
      });
      ...
      return {
        ...
        userId: user.id,
        message: `Un email de confirmation a été envoyé a cette adresse email : ${user.email}, clique sur le lien ${validationLink} dans le mail afin valider ton compte !`
      };
    }
  }
};
