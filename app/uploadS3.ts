...
const uploadToS3 = async (filename: string, stream: Body, companyId: string, productId: string | null): Promise<UploadS3> => {
...
const uploadToS3SaveAsCompanyAvatarOrCover = async (filename: string, stream: Body, companyId: string, islogo: boolean): Promise<void> => {
  const {image} = await uploadToS3(filename, stream, companyId, null);
  const update = islogo ? {logoId: image.id} : {coverId: image.id};
  CompanyModel.update(update, {where: {id: companyId}});
};

const uploadToS3SaveAsProductCover = async (filename: string, stream: Body, companyId: string, productId: string): Promise<void> => {
  const {image} = await uploadToS3(filename, stream, companyId, null);
  ProductModel.update({coverId: image}, {where: {id: productId}});
};

export  {uploadToS3SaveAsCompanyAvatarOrCover, uploadToS3SaveAsProductCover};